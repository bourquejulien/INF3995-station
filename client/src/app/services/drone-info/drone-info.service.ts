...
export class DroneInfoService {
    ...
    getLatestMetric(): Observable<Metric> {
        return this.httpClient.get<Metric>(`${environment.serverURL}/drone-info/latestMetric`).pipe(catchError(this.handleError));
    }

    getLatestMap(): Observable<Map<string, MapMetric>> {
        return this.httpClient.get<Map<string, MapMetric>>(`${environment.serverURL}/drone-info/latestMap`).pipe(catchError(this.handleError));
    }
    ...
    getLogs(missionId: string, sinceTimestamp?: number): Observable<Log[]> {
        ...
        return this.httpClient.get<Log[]>(`${environment.serverURL}/drone-info/logs`, { params: queryParams }).pipe(catchError(this.handleError));
    }
