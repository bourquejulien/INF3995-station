...
def test_get_logs_with_timestamp(client, app, mocker):
    ...
    with app.container.logging_service.override(logging_service_mock):
        response = client.get(
            url_for("drone-info.get_logs"), query_string={"since_timestamp": 1000, "mission_id": "test"}
        )
...
def test_get_logs_without_timestamp(client, app, mocker):
    ...
    with app.container.logging_service.override(logging_service_mock):
        response = client.get(url_for("drone-info.get_logs"), query_string={"mission_id": "test"})
